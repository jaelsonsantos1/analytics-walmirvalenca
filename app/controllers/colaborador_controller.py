...
class ColaboradorController:
    ...
    def get_all_colaboradores(page, page_size):
        colaboradores = ColaboradorModel.get_all_colaboradores(page, page_size)
        
        colaboradores_df = pd.DataFrame(colaboradores, columns=['id', 'nome', 'idade', 'id_gestor', 'gestor', 'id_setor', 'setor', 'created_at'])
        colaboradores_df['created_at'] = pd.to_datetime(colaboradores_df['created_at']).dt.strftime('%d/%m/%Y %H:%M:%S')
    ...
    def get_all_avaliacoes():
        ...
        avaliacoes_lista = pd.DataFrame(avaliacoes, columns=['ID', 'Colaborador', 'Avaliador', 'Setor', 'Produtividade', 'Atingiu Meta', 'Comentário produtividade', 'Engajamento', 'Comentário engajamento'])

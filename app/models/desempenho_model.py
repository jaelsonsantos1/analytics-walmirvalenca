...
class DesempenhoModel:
    ...
    def get_all_desempenhos():
        db = Database()
        db.cursor.execute(f"""
            SELECT C.nome_colaborador, E.nota_avaliacao, E.comentario, G.nome_gestor, S.nome_setor, E.created_at FROM engajamento as E
    ...
    def delete_desempenho(id):
        db = Database()
        db.cursor.execute(f"""
            DELETE FROM engajamento WHERE id = '{id}';
        """)
        db.connection.commit()
        db.close_connection()
        return id
